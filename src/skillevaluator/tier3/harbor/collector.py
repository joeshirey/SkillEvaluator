...
def _strip_arm_suffix(value: str) -> str:
    """Remove the SkillEvaluator with-skill/without-skill arm suffix.

    Harbor task names generated by ``_write_task_toml`` are
    ``nvidia/skillevaluator-{entry_id}-{arm}`` (see adapter.py), so Harbor's
    own ``result.json['task_name']`` carries this suffix too. Strip it before
    reconciling a Harbor-derived identifier back to the bare scenario id.
    """
...
def _canonical_case_id(value: str, expected_case_ids: set[str] | None = None) -> str:
    ...
    if expected_case_ids and value in expected_case_ids:
        return value
    stripped = _strip_arm_suffix(_strip_attempt_suffix(value))
